...
app.post("/publish", async (req, res) => {
    ...
    // 2. 将数据存入Lose数据表里
    try {
        const { type, username, userAvatar, classify1, classify2, name, date, region, phone, desc, imgList, time, openid } = req.body;
        await Lose.create({
            type, username, userAvatar, classify1, classify2, name, date, region, phone, desc, imgList, time, openid
        });
        res.send("success");
    } catch (error) {
        console.log(error);
